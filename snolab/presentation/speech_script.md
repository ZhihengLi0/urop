...
**English**: This slide is step five — alignment. Each measured trace is shifted by its own fitted pretrigger minus the nominal 16050 — a pure translation, nothing about the waveform is regenerated. The dotted vertical line is the common pretrigger everything is aligned to. Once aligned, I draw all the fitted curves from that common starting point — these two fan plots. On the left, all the physical fits: you can see some clearly separated, slowly-falling stragglers spreading off the main bundle. On the right, after selecting on NRMSE, only one very tight, consistent shape family remains. It's aligning away the trigger jitter and then dropping the bad fits that leaves this tight bundle — and that's what makes a well-defined template possible. Exactly how the NRMSE threshold is set is the next slide.
