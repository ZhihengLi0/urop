...
| file | what |
|---|---|
| `SNOLAB_R4_pulse_templates_20260715.pptx` | 12 slides (11 main + 1 backup), 16:9, English; each slide has English speaker notes |
...
→ backup: weak detectors (Z22 example).
...
11 main slides + 1 backup. The echo-trigger (slow-rise) and τ_rise-ceiling slides were dropped per Prof. Saab; the τ_rise ≤ 0.3 ms cut survives as a keyword on the PCA slide (removal fractions relative to the NRMSE step: Z7 1.6% — mostly the bad channel PDS2 — vs 55–74% on the weak zips; see `lp_fit_align/scripts/trise_cut_stats.py`). Slide 7 (slow-fall follow-up) is the designated skip-slide if running long.
