#!/usr/bin/env python3
# coding: utf-8
# Fifth-iteration template generation for SNOLAB Run 4.
...
#
# NxM synthetic traces use one exact, fixed pretrigger sample. Raw traces with
# a significant negative excursion are rejected before NxM fitting and PCA.
#
# Usage: python template_single_zip_v5.py --det <zip_number>
...
# ── NxM: per-event 4-exp fit → fixed-pretrigger traces → PCA ────────────────
#
# Every synthetic trace begins at exactly CANONICAL_PT. The peak is deliberately
# not aligned: different fitted time constants may put it at a different later
# sample, which is physical shape variation for PCA to describe.

N_COMPONENTS  = 5
MAX_NXM       = 300
CANONICAL_PT  = 16250                     # exact first-pretrigger sample
NXM_RMSE_MAX  = 0.15
DIAG_PER_CHAN = 3
# Input traces are normalized to a positive peak of 1. Baseline noise naturally
# crosses zero, so reject only a significant negative pulse/undershoot. Clipping
# is not used because it would create artificial PCA structure.
NEGATIVE_FRACTION = float(os.environ.get('R4_NEGATIVE_FRACTION', '0.05'))
if not 0.0 < NEGATIVE_FRACTION < 1.0:
    raise ValueError('R4_NEGATIVE_FRACTION must be strictly between 0 and 1')
...
def make_fixed_pretrigger_synth(a1, a2, a3, t1, t2, t3, t4):
    """Generate a synthetic trace whose pretrigger is exactly CANONICAL_PT."""
    synth = four_exp_fit(
        x_full_nxm, a1, a2, a3, t1, t2, t3, t4, 0.0, float(CANONICAL_PT))
    # four_exp_fit must keep every sample through the fixed pretrigger exactly
    # at baseline. Fail loudly if a future edit violates this invariant.
    if np.any(synth[:CANONICAL_PT + 1] != 0.0):
        raise RuntimeError('fixed-pretrigger invariant failed')
    return synth

nxm_synth_all    = []
nxm_synth_bychan = {c: [] for c in chan_names}

print(f"\n── NxM per-event fitting (fixed pretrigger={CANONICAL_PT}) ──")
print(f"   Rejecting traces with min/positive-peak < -{NEGATIVE_FRACTION:.3f}")
...
for chan in chan_names:
    ...
    for i, tr in enumerate(traces[:MAX_NXM]):
        if not np.all(np.isfinite(tr)):
            n_reject += 1
            continue
        if np.min(tr) < -NEGATIVE_FRACTION:
            n_negative += 1
            continue
        ...
        # Canonical synthetic trace with one exact pretrigger for every event.
        synth = make_fixed_pretrigger_synth(a1, a2, a3, t1, t2, t3, t4)
        peak_s = np.max(synth)
    ...
    print(f"  {chan}: {n_accept} accepted, {n_reject} fit/shape rejected, "
          f"{n_negative} negative-excursion rejected")
...
if len(nxm_synth_all) <= N_COMPONENTS:
    ...
    if (not np.all(np.isfinite(nxm_arr)) or np.min(nxm_arr) < -1e-12 or
            np.any(nxm_arr[:, :CANONICAL_PT + 1] != 0.0)):
        raise RuntimeError(
            'agnostic PCA input failed fixed-pretrigger/positivity checks')
    ...
    fig_pca.suptitle(f'Zip{det_tmpl} — Agnostic NxM PCA '
                     '(fixed pretrigger, all channels pooled)',
                     fontsize=12)
...
for chan in chan_names:
    ...
    if (not np.all(np.isfinite(nxm_chan_arr)) or
            np.min(nxm_chan_arr) < -1e-12 or
            np.any(nxm_chan_arr[:, :CANONICAL_PT + 1] != 0.0)):
        raise RuntimeError(
            f'{chan} PCA input failed fixed-pretrigger/positivity checks')
    pca_ch  = PCA(N_COMPONENTS, svd_solver='randomized', random_state=42).fit(nxm_chan_arr)
    ...
    axes_pca[-1].set_xlabel('Time (ms)')
    axes_pca[0].set_xlim(lo_ms, hi_ms)
    fig_pca.suptitle(f'Zip{det_tmpl}/{chan} — Specific NxM PCA (fixed pretrigger)',
                     fontsize=12)
    fig_pca.tight_layout()
