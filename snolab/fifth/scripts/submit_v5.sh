#!/bin/bash
# Fifth-iteration template generation: agnostic + specific NxM (fixed pretrigger).
# Each per-zip job writes to both agnostic/ and specific/ subdirs.
set -euo pipefail

REPO="$HOME/urop/snolab/fifth"
DATE="$(date +%Y%m%d)"
RUN_DIR="$REPO/run/r4_v5_${DATE}"
...
for det in 1 4 6 7 10 15 16 18; do
    jid=$(sbatch --parsable \
        ...
        --wrap="singularity exec -B \$HOME,\$MSIPROJECT/shared/ $IMAGE \
                python3 -u $SCRIPTS/template_single_zip_v5.py --det $det")
    job_ids+=("$jid")
...
shape_ag=$(sbatch --parsable \
    ...
    --export="ALL,R4_RUN_DIR=$RUN_DIR" \
    --wrap="singularity exec -B \$HOME,\$MSIPROJECT/shared/ $IMAGE \
            python3 -u $SCRIPTS/plot_templates_v5.py --mode agnostic")
echo "Submitted agnostic shape plot: $shape_ag"

shape_sp=$(sbatch --parsable \
    --job-name="r4v5_plot_sp" \
    --dependency="afterok:$dependency" \
    --time=0:30:00 \
    --ntasks=1 --mem=16g --partition=msismall \
    ...
    --export="ALL,R4_RUN_DIR=$RUN_DIR" \
    --wrap="singularity exec -B \$HOME,\$MSIPROJECT/shared/ $IMAGE \
            python3 -u $SCRIPTS/plot_templates_v5.py --mode specific")
