...
PRETRIG_CHECK_LO  = 15000   # samples before pulse to check for noise
PRETRIG_CHECK_HI  = 16000
PRETRIG_STD_MAX   = 0.30    # max allowed std relative to peak=1.0
...
for det in ZIPS:
    ...
    for traces_dict, fname_tag, suptitle in [
        (traces_corr,
         'section3_corrected',
         'Analytical plot of all events — PTOFdelay corrected, peak-normalised'),
        (traces_uncorr,
         'section3_uncorrected',
         'Analytical plot of all events — no alignment (uncorrected), peak-normalised'),
    ]:
        ...
        for idx, ch in enumerate(chans_have):
            ...
            ch_traces_raw = traces_dict.get(ch, [])
            ch_traces = []
            n_removed = 0
            for tr in ch_traces_raw:
                tr = np.asarray(tr, dtype=float)
                pretrig = tr[PRETRIG_CHECK_LO:PRETRIG_CHECK_HI]
                if len(pretrig) > 0 and np.std(pretrig) > PRETRIG_STD_MAX:
                    n_removed += 1
                    continue
                ch_traces.append(tr)
            if n_removed:
                print(f"  {ch}: removed {n_removed}/{len(ch_traces_raw)} noisy traces")
            ...
            for ax, tag in [(ax_full, 'full'), (ax_zoom, 'zoom')]:
                ax.axvline(x=t_axis[CANONICAL_PT], color='gray',
                           lw=0.8, ls='--', alpha=0.5)
                ax.set_title(f'{ch} (n={len(ch_traces)}/{len(ch_traces_raw)}) [{tag}]', fontsize=8)
