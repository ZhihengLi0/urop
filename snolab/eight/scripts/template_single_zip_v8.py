#!/usr/bin/env python3
# coding: utf-8
# Eighth-iteration template generation for SNOLAB Run 4.
...
#
# Changes from v7:
#   1. NxM per-event fitting is extended to four exponentials:
#        Step 1: 2-exp → fixes t1 (rise) and t2 (primary fall)
#        Step 2: 3-exp → fixes t3 (secondary fall), holding t1/t2 exactly constant
#        Step 3: 4-exp → adds t4 (slow fall), holding t1/t2/t3 exactly constant
#   2. 1x1 average-template fitting: after the best 3-exp solution, t1/t2/t3 are
#      fixed and a 4-exp candidate is attempted; BIC selects among 2/3/4-exp.
#   3. All 1x1 analytical traces are re-pinned to CANONICAL_PT before being
#      written to ROOT, so channel pretrigger offsets do not propagate.
#
...
#
# Usage: python template_single_zip_v8.py --det <zip_number>
...
print(f"   4 kHz filtered → 2-exp fixes t1/t2 → 3-exp fixes t3 → 4-exp adds t4 → mean + PCA")
...
if len(nxm_synth_all) <= PCA_COMPONENTS:
    ...
    fig_pca.suptitle(f'Zip{det_tmpl} — Agnostic NxM mean + PCA '
                     '(4 kHz, PTOFdelay aligned, 4-exp, all channels pooled)',
                     fontsize=12)
