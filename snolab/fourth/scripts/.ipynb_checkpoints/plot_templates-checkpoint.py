...
# ─── Cross-zip time constant comparison (t1 rise, t2 fall) ───────────────────
# Load time_constants_zip{det}.json produced by template_single_zip.py
# For each channel present in multiple zips, plot t1 and t2 side by side.

print("\n── Cross-zip time constant comparison ──")

tc_by_zip = {}
for det in ZIPS:
    tc_path = os.path.join(ROOT_DIR, f'time_constants_zip{det}.json')
    if not os.path.exists(tc_path):
        print(f"  Zip{det}: time_constants JSON not found, skipping")
        continue
    with open(tc_path) as fj:
        tc_by_zip[det] = json.load(fj)

if tc_by_zip:
    # Collect all channel names that appear in at least 2 zips
    all_chans_seen = {}
    for det, tc in tc_by_zip.items():
        for ch in tc:
            all_chans_seen.setdefault(ch, []).append(det)

    chans_to_plot = sorted(
        ch for ch, dets in all_chans_seen.items() if len(dets) >= 1
    )

    colors_zip = {det: col for det, col in
                  zip(ZIPS, plt.cm.tab10(np.linspace(0, 1, len(ZIPS))))}
    x_pos = np.arange(len(ZIPS))
    bar_w = 0.6

    for ch in chans_to_plot:
        # gather t1/t2 per zip (None if zip doesn't have this channel)
        t1_vals = []
        t2_vals = []
        labels  = []
        colors  = []
        for det in ZIPS:
            tc = tc_by_zip.get(det, {})
            entry = tc.get(ch)
            if entry is not None:
                t1_vals.append(entry['t1'] * 1e3)   # convert s → ms
                t2_vals.append(entry['t2'] * 1e3)
                labels.append(f'Zip{det}')
                colors.append(colors_zip[det])

        if not t1_vals:
            continue

        xi = np.arange(len(labels))
        fig_tc, axes_tc = plt.subplots(1, 2, figsize=(12, 4))

        axes_tc[0].bar(xi, t1_vals, color=colors, width=bar_w, edgecolor='k', lw=0.7)
        axes_tc[0].set_xticks(xi)
        axes_tc[0].set_xticklabels(labels, rotation=30, fontsize=9)
        axes_tc[0].set_ylabel('Rise time t1 (ms)')
        axes_tc[0].set_title(f'{ch} — Rise time t1 across zips')
        axes_tc[0].grid(axis='y', alpha=0.4)
        for j, v in enumerate(t1_vals):
            axes_tc[0].text(j, v * 1.02, f'{v:.3f}', ha='center',
                            va='bottom', fontsize=8)

        axes_tc[1].bar(xi, t2_vals, color=colors, width=bar_w, edgecolor='k', lw=0.7)
        axes_tc[1].set_xticks(xi)
        axes_tc[1].set_xticklabels(labels, rotation=30, fontsize=9)
        axes_tc[1].set_ylabel('Fall time t2 (ms)')
        axes_tc[1].set_title(f'{ch} — Fall time t2 across zips')
        axes_tc[1].grid(axis='y', alpha=0.4)
        for j, v in enumerate(t2_vals):
            axes_tc[1].text(j, v * 1.02, f'{v:.2f}', ha='center',
                            va='bottom', fontsize=8)

        fig_tc.suptitle(
            f'Channel {ch} — Rise & Fall time comparison across all Zips',
            fontsize=12)
        fig_tc.tight_layout()
        out_tc = os.path.join(OUTDIR, f'timeconstants_{ch}_all_zips.png')
        fig_tc.savefig(out_tc, dpi=150)
        plt.close(fig_tc)
        print(f"  Saved: {out_tc}")

    # Summary: all channels' t1 and t2 in one figure per time constant
    for tc_key, tc_label, unit_scale in [('t1', 'Rise time t1', 1e3),
                                          ('t2', 'Fall time t2', 1e3)]:
        fig_s, ax_s = plt.subplots(figsize=(max(12, len(chans_to_plot) * 1.1), 5))
        width  = 0.8 / len(ZIPS)
        for k, det in enumerate(ZIPS):
            tc = tc_by_zip.get(det, {})
            vals  = [tc[ch][tc_key] * unit_scale if ch in tc else np.nan
                     for ch in chans_to_plot]
            xi_s  = np.arange(len(chans_to_plot)) + k * width
            ax_s.bar(xi_s, vals, width=width, label=f'Zip{det}',
                     color=colors_zip[det], edgecolor='k', lw=0.5, alpha=0.85)
        ax_s.set_xticks(np.arange(len(chans_to_plot)) + 0.4 * (1 - 1/len(ZIPS)))
        ax_s.set_xticklabels(chans_to_plot, rotation=45, fontsize=9)
        ax_s.set_ylabel(f'{tc_label} (ms)')
        ax_s.set_title(f'{tc_label} — All channels & all zips')
        ax_s.legend(ncol=4, fontsize=8)
        ax_s.grid(axis='y', alpha=0.4)
        fig_s.tight_layout()
        out_s = os.path.join(OUTDIR, f'timeconstants_summary_{tc_key}_all_zips.png')
