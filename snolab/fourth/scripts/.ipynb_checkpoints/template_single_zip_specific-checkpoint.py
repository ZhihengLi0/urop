...
# ── Write templates to ROOT ───────────────────────────────────────────────────
os.makedirs(ROOT_DIR, exist_ok=True)
output_file = os.path.join(ROOT_DIR, f"Templates_SNOLAB_R4_zip{det_tmpl}_NxM_specific.root")
...
# ── NxM templates (specific): per-channel PCA ─────────────────────────────────
#
# Specific version: PCA is run independently per channel.
# Each channel gets its own set of NxM components derived only from traces
# of that channel — avoids inter-channel shape mixing that causes pre-pulse dips.
...
nxm_synth_bychan = {c: [] for c in chan_names}  # per-channel synthetic traces
...
# ── Per-channel PCA on full-length synthetic traces ───────────────────────────
...
for chan in chan_names:
    ...
    if len(synth_list) < N_COMPONENTS:
        print(f"  {chan}: only {len(synth_list)} traces — need {N_COMPONENTS}, skipping NxM")
        continue

    nxm_arr = np.array(synth_list)
    pca_chan = PCA(N_COMPONENTS, svd_solver='full').fit(nxm_arr)
    PC_chan  = pca_chan.components_
    var_chan = pca_chan.explained_variance_ratio_

    print(f"\n  {chan}: PCA on {len(synth_list)} synthetic traces")
    for i, v in enumerate(var_chan):
        print(f"    PC{i}: {v*100:.2f}% variance")

    # PCA components plot per channel
    fig_pca, axes_pca = plt.subplots(N_COMPONENTS, 1,
                                     figsize=(14, 3 * N_COMPONENTS), sharex=True)
    for i in range(N_COMPONENTS):
        axes_pca[i].plot(t_ms_pca, PC_chan[i], lw=0.9)
        axes_pca[i].set_title(f'PC{i}  ({var_chan[i]*100:.2f}%)', fontsize=10)
        axes_pca[i].set_ylabel('Amplitude')
        axes_pca[i].grid(alpha=0.3)
    axes_pca[-1].set_xlabel('Time (ms)')
    axes_pca[0].set_xlim(lo_ms, hi_ms)
    fig_pca.suptitle(
        f'Zip{det_tmpl}/{chan} — NxM PCA Components (specific, {N_COMPONENTS})',
        fontsize=12)
    fig_pca.tight_layout()
    pca_path = os.path.join(PLOT_DIR, f'nxm_pca_components_zip{det_tmpl}_{chan}.png')
    fig_pca.savefig(pca_path, dpi=150)
    plt.close(fig_pca)

    # Variance explained plot per channel
    fig_var, ax_var = plt.subplots(figsize=(6, 4))
    ax_var.plot(range(N_COMPONENTS), var_chan * 100, 'o-', color='steelblue')
    ax_var.set_xlabel('Component index')
    ax_var.set_ylabel('Explained variance (%)')
    ax_var.set_title(f'Zip{det_tmpl}/{chan} NxM — Explained variance')
    ax_var.set_yscale('log')
    ax_var.grid(alpha=0.4)
    fig_var.tight_layout()
    var_path = os.path.join(PLOT_DIR, f'nxm_variance_zip{det_tmpl}_{chan}.png')
    fig_var.savefig(var_path, dpi=120)
    plt.close(fig_var)

    # Write NxM templates for this channel
    for i in range(N_COMPONENTS):
        hname  = f"{chan}nxm{i}"
...
print(f"\nNxM specific: {N_COMPONENTS} components written for {n_chan_written}/{len(chan_names)} channels.")
