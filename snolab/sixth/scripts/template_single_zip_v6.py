#!/usr/bin/env python3
# coding: utf-8
# Sixth-iteration template generation for SNOLAB Run 4.
...
#
# All raw traces receive a 4 kHz low-pass filter before 1x1/NxM fitting. NxM
# follows the reference notebook's two-exponential, fixed-pretrigger method.
...
#
# Usage: python template_single_zip_v6.py --det <zip_number>
...
if FORCE_RERUN or not os.path.exists(CACHE_FILE):
    ...
    with open(CACHE_FILE, 'wb') as f:
        pickle.dump({'cache_version': CACHE_VERSION,
                     ...
                     'negative_rejected': negative_rejected}, f)
...
# pretrigger consistently, and run PCA on synthetic fit waveforms.  Per the
# sixth-iteration requirement, event amplitudes are preserved rather than
...
def make_fixed_pretrigger_synth(amp, t1, t2):
    ...
    # two_exp_fit must keep every sample through the fixed pretrigger exactly
...
print(f"   4 kHz filtered input → positive 2-exp synthetic waveforms → mean + PCA")
...
for chan in chan_names:
    ...
    for i in selected_indices:
        ...
        try:
            with warnings.catch_warnings():
                warnings.simplefilter('ignore')
                popt_ev, _ = curve_fit(two_exp_fit, x_nxm_fit, y_region,
                    p0=p0_ev,
        ...
        amp,t1,t2,bl,pt_ev = popt_ev
        y_fit_win = two_exp_fit(x_nxm_fit, *popt_ev)
        rmse      = float(np.sqrt(np.mean((y_region - y_fit_win)**2)))
        nrmse     = rmse / trace_peak

        if amp <= 0 or t2 <= t1 * 1.05:
            n_reject += 1; continue
        if t1 >= T1_MAX * 0.98 or t2 >= T_DECAY_MAX * 0.98:
            n_reject += 1; continue
        ...
        if diag_done < DIAG_PER_CHAN:
            y_fit_disp = two_exp_fit(x_full_nxm, *popt_ev) - bl
        ...
        synth = make_fixed_pretrigger_synth(amp, t1, t2)
        peak_s = np.max(synth)
...
if len(nxm_synth_all) <= PCA_COMPONENTS:
    ...
    fig_pca.suptitle(f'Zip{det_tmpl} — Agnostic NxM mean + PCA '
                     '(4 kHz, fixed pretrigger, all channels pooled)',
                     fontsize=12)
