...
Unlike the fifth iteration, this does not compare individual exponential fit
parameters across different models.  Rise is measured from 10% to 90% of the
fitted pulse peak; fall is measured from the peak to 1/e.  These definitions are
stable for both two- and three-exponential fits.
"""
...
with open(csv_path, 'w', newline='') as handle:
    ...
    writer.writerow(['Channel', 'Zip', 'Model', 'Rise10_90_ms', 'Fall1e_ms'])
...
for chan in ALL_CHANS:
    ...
    fig.suptitle(f'Channel {chan} — Waveform Rise & Fall Comparison (6th iteration)',
                 fontsize=13)
    fig.tight_layout()
...
for key, ylabel, filename in [
    ...
    for ax, chan in zip(axes.flat, ALL_CHANS):
        ...
        ax.set_ylabel(ylabel)
        ax.grid(axis='y', alpha=0.3)
    fig.suptitle(f'{ylabel} — all channels and zips (6th iteration)', fontsize=14)
    fig.tight_layout()
