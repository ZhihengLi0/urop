...
**中文**：下面看一个例子：通道 PBS1，事件 30646，只画出脉冲附近这 1.3 毫秒，横轴从触发前 0.35 毫秒到触发后 0.95 毫秒。同一条波形，可以用 ADC 计数、电流和功率三种单位来读。上半部分是脉冲，有三条纵轴。左边是 ADC 计数，已经减去基线，是仪器直接记录的数。右边第一条是电流，单位微安，1 个 ADC 等于 0.318 纳安。右边第二条是功率，单位飞瓦，用上一页的公式换算。下半部分是功率随时间的变化。这里说明一点：双指数拟合用的是 100 kHz 低通滤波以后的波形，也就是蓝线，不是原始采样点。原始采样点只是画出来做对比。接下来两页分别讲上半部分和下半部分。

**English**: Now an example: channel PBS1, event 30646, showing only 1.3 milliseconds around the pulse, from 0.35 milliseconds before the trigger to 0.95 after. **The same trace can be read in three units: as ADC counts, as current, and as power.** The top panel is the pulse, with three vertical axes. The left axis is ADC counts with the baseline subtracted, which is what the instrument records. The first right axis is current in microamps; one ADC is 0.318 nanoamps. The second right axis is power in femtowatts, converted with the formula from the last slide. The bottom panel shows the power over time. **One note: the two-exponential fit is done on the trace after a 100 kilohertz low-pass filter, which is the blue line, not on the raw samples.** The raw samples are drawn only for comparison. The next two slides explain the top panel and the bottom panel.
