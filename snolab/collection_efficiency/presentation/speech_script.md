...
**中文**：从一个事件扩展到所有事件。这里还是 PBS1，每个事件做一次拟合和一次积分，得到一个能量。横轴是能量，每格 10 电子伏，纵轴是事件数。红色来自拟合，灰色轮廓来自官方窗口对原始波形的积分，两者的峰位只差 0.2%。峰在 285 电子伏，宽度 42 电子伏，也就是 15%。要注意，这些事件的真实能量都一样，但分布的宽度有 15%。分布的右边有一条长尾，一直到 600 电子伏左右，这些是离 PBS1 比较近的事件。所以一个通道的宽度主要来自事件发生的位置，而不是噪声。下一页会看到证据：把通道加起来以后宽度会明显变小。虚线是只对中间部分做的高斯拟合，所以 μ 是峰的位置，不是平均值。
