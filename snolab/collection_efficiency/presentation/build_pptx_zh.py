...
# ------------------------------------------------------------------ 10 求和
s = new("各通道求和：30.5%", "每个 K 线事件被 TES 吸收的总能量")
pic(s, "hist_sum.png", L, T, Inches(8.8), Inches(5.9))
lines(s, [
    "!10 个通道求和，1827 个事件：3162 eV = 30.5%，宽度降到 7.5% —— 位置效应抵消了。",
    "PDS2 不算在内：它只有 42% 的事件能拟合上，而这些事件都发生在它附近。",
    "!收集效率 30.5% ± 2.3%（σ/μ = 7.5%）。",
...
lines(s, [
    ...
    "!Z7 把一个 10.37 keV 事件的 30.5% ± 2.3%（σ/μ = 7.5%）吸收进 TES，10 个通道求和。   CDMS 文档在 R37 CUTE tower 上得到的是 26–41%。",
    "还没落实：只做了 Z7 · Z7 的偏压没有确认（按 0 V 算） · 没有 dI/dV · PDS2 没有算进去 · 12.5% 的事件在任何通道都拟合不上。",
...
lines(s, [
    ...
    "两者只有二次项不同：能量差 1.7%（PBS1，15 个事件的中位数）—— 远小于单通道 15% 的宽度和求和后 7.5% 的宽度。",
