...
if RUN_DIR:
    ...
    CACHE_DIR = "."
    ROOT_DIR  = "root_files"
    PLOT_DIR  = "template_plots"

if RUN_DIR:
    PLOT_DIR = os.path.join(RUN_DIR, "template_plots")
...
CACHE_VERSION = 3          # bumped: now stores raw_sample for diagnostic plots
ALIGN_PEAK_LO = 15000
ALIGN_PEAK_HI = 18000
RAW_SAMPLE_N  = 5          # number of raw traces to keep per channel for NxM diagnostics
...
if not FORCE_RERUN and os.path.exists(CACHE_FILE):
    ...
    if cache.get('cache_version') == CACHE_VERSION:
        ...
        print("Loaded:")
        for c in chans:
            print(f"  {c}: {len(channel_traces[c])} traces, {len(raw_sample.get(c,[]))} raw samples")
...
if FORCE_RERUN or not os.path.exists(CACHE_FILE):
    ...
    pf_traces      = []
    raw_sample     = {c: [] for c in chans}   # baseline-sub only, unfiltered, aligned
    ...
    for series, evnums in sel_events.items():
        ...
        for event in events_list:
            ...
            for chan in chans:
                ...
                y_raw_al = np.roll(y_sub,  shift)   # unfiltered, same alignment
                ...
                # keep a few raw (unfiltered) traces for later diagnostic plots
...
# ── Save 1x1 time constants for cross-zip comparison ──────────────────────────
tc_out = {}
...
tc_path = os.path.join(ROOT_DIR, f'time_constants_zip{det_tmpl}.json')
with open(tc_path, 'w') as fj:
    json.dump(tc_out, fj, indent=2)
print(f"Time constants saved: {tc_path}")

# ── NxM templates: per-event 4-exp fit → canonical synthetic traces → PCA ─────
#
# Algorithm (following Yan Liu NxM methodology):
#   1. Each event's LP-filtered aligned trace is fit with 4-exp to extract
#      time constants (t1 rise, t2/t3/t4 fall).
#   2. Quality cuts: reject events with bad fits (amplitude, RMSE, pretrigger).
#   3. Diagnostic plots: raw (unfiltered), LP filtered, 4-exp fit — one figure
#      per sample event, saved to PLOT_DIR.
#   4. Generate canonical synthetic trace: same time constants, but amp=1,
#      baseline=0, pretrigger fixed → eliminates amplitude/offset variation,
#      PCA then captures only pulse-shape variation.
#   5. Before/after pretrigger alignment plot.
#   6. PCA on full 32768-sample synthetic traces (5 components).
#   7. Write PCA components as NxM templates to ROOT.

N_COMPONENTS  = 5
MAX_NXM       = 300          # max events per channel fed to training set
CANONICAL_PT  = float(tracelength // 2)   # 16384 — canonical pretrigger
NXM_RMSE_MAX  = 0.15         # reject event if 4-exp RMSE > this
DIAG_PER_CHAN = 3             # diagnostic plots saved per channel

x_full = np.arange(tracelength, dtype=float)

def nxm_p0(chan):
    """Initial 4-exp guess from the 1x1 average-template fit."""
...
nxm_synth_all  = []    # all accepted synthetic traces (full length)
nxm_synth_bychan = {c: [] for c in chan_names}  # for alignment plot (per chan)

# Fitting is done on the windowed region [WIN_START:end] to focus on the pulse
# and its full decay tail — same window as the 1x1 average-template fit.
# The full 32768-sample synthetic trace is generated separately for PCA.
x_nxm_fit  = np.arange(WIN_START, tracelength, dtype=float)   # fit window
x_full_nxm = np.arange(tracelength, dtype=float)              # full trace

print(f"\n── NxM per-event fitting (4-exp, window [{WIN_START}:{tracelength}], "
      f"synthetic full {tracelength}-sample) ──")
...
for chan in chan_names:
    ...
    p0 = nxm_p0(chan)
    if p0 is None:
        print(f"  {chan}: no 1x1 params for initial guess")
        continue
    ...
    for i, tr in enumerate(traces[:MAX_NXM]):
        y_region = tr[WIN_START:]   # pulse + full decay tail
        try:
            with warnings.catch_warnings():
                warnings.simplefilter('ignore')
                popt_ev, _ = curve_fit(
                    four_exp_fit, x_nxm_fit, y_region,
                    p0=p0,
        ...
        # RMSE on the windowed region only (pulse + decay tail)
        ...
        # Full-length fit curve for display (baseline subtracted so it overlays tr)
        y_fit_full_disp = four_exp_fit(x_full_nxm, *popt_ev) - bl

        # Diagnostic plot: raw / LP filtered / 4-exp fit
        if diag_done < DIAG_PER_CHAN:
            fig_d, ax_d = plt.subplots(figsize=(13, 4))
            t_ms_full = x_full_nxm / samplerate * 1e3
            lo_ms = (CANONICAL_PT - 2000) / samplerate * 1e3
            hi_ms = (CANONICAL_PT + 8000) / samplerate * 1e3
            if i < len(raw_samps):
                ax_d.plot(t_ms_full, raw_samps[i], color='gray', lw=0.7,
                          alpha=0.9, label='Raw (unfiltered, norm.)')
            ax_d.plot(t_ms_full, tr, color='steelblue', lw=1.0,
                      label='LP filtered (norm.)')
            ax_d.plot(t_ms_full, y_fit_full_disp, color='red', lw=1.5, ls='--',
                      label=f'4-exp fit  RMSE={rmse:.4f} (window [{WIN_START}:])')
            ax_d.set_xlim(lo_ms, hi_ms)
            ax_d.set_xlabel('Time (ms)')
            ax_d.set_ylabel('Amplitude (norm.)')
            ax_d.set_title(
                f'Zip{det_tmpl}/{chan} event {i} — Raw / LP filtered / 4-exp fit\n'
                f't1={t1*1e3:.3f} ms  t2={t2*1e3:.2f} ms  '
                f't3={t3*1e3:.2f} ms  t4={t4*1e3:.2f} ms')
            ax_d.legend(fontsize=8)
            ax_d.grid(alpha=0.3)
            fig_d.tight_layout()
            dpath = os.path.join(PLOT_DIR,
                                 f'nxm_diag_zip{det_tmpl}_{chan}_ev{i}.png')
            fig_d.savefig(dpath, dpi=120)
            plt.close(fig_d)
            diag_done += 1

        # Canonical synthetic trace on full 32768 samples: only time constants kept,
        # amplitude normalized to 1, baseline=0, pretrigger fixed at CANONICAL_PT
        synth = four_exp_fit(x_full_nxm, a1, a2, a3, t1, t2, t3, t4,
                             0.0, CANONICAL_PT)
        peak_s = np.max(synth)
        if peak_s <= 0:
            n_reject += 1; continue
...
# ── Before / after pretrigger alignment plot ──────────────────────────────────
...
if demo_chan:
    ...
    axes_al[0].set_title(f'Zip{det_tmpl}/{demo_chan} — Before canonical alignment\n'
                         '(LP filtered, OF-delay aligned, peaks near CANONICAL_PT)')
    axes_al[0].set_xlabel('Time (ms)')
    axes_al[0].set_ylabel('Amplitude (norm.)')
    axes_al[0].grid(alpha=0.3)

    for tr in synth_30:
        axes_al[1].plot(t_ms_al, tr, alpha=0.3, lw=0.6, color='darkorange')
    axes_al[1].set_xlim(lo_ms, hi_ms)
    axes_al[1].set_title(f'Zip{det_tmpl}/{demo_chan} — After canonical alignment\n'
                         f'(synthetic, pretrigger fixed at {int(CANONICAL_PT)})')
    axes_al[1].set_xlabel('Time (ms)')
    axes_al[1].grid(alpha=0.3)

    fig_al.suptitle(f'Zip{det_tmpl} NxM — Pretrigger alignment', fontsize=12)
    fig_al.tight_layout()
    al_path = os.path.join(PLOT_DIR, f'nxm_alignment_zip{det_tmpl}.png')
    fig_al.savefig(al_path, dpi=150)
    plt.close(fig_al)
    print(f"Alignment plot saved: {al_path}")

# ── PCA on full-length synthetic traces ───────────────────────────────────────
if len(nxm_synth_all) < N_COMPONENTS:
    print(f"WARNING: only {len(nxm_synth_all)} synthetic traces — "
          f"need at least {N_COMPONENTS} for PCA. NxM skipped.")
else:
    nxm_arr = np.array(nxm_synth_all)
    pca = PCA(N_COMPONENTS, svd_solver='full').fit(nxm_arr)
    PC  = pca.components_          # shape (N_COMPONENTS, tracelength)
    var = pca.explained_variance_ratio_

    print(f"\nPCA on {len(nxm_synth_all)} synthetic traces "
          f"({N_COMPONENTS} components, full {tracelength}-sample traces)")
    ...
    for i in range(N_COMPONENTS):
        axes_pca[i].plot(t_ms_pca, PC[i], lw=0.9)
        axes_pca[i].set_title(f'PC{i}  ({var[i]*100:.2f}%)', fontsize=10)
        axes_pca[i].set_ylabel('Amplitude')
        axes_pca[i].grid(alpha=0.3)
    axes_pca[-1].set_xlabel('Time (ms)')
    axes_pca[0].set_xlim(lo_ms, hi_ms)
    fig_pca.suptitle(
        f'Zip{det_tmpl} — NxM PCA Components ({N_COMPONENTS}, '
        f'full {tracelength}-sample)', fontsize=12)
    fig_pca.tight_layout()
    pca_path = os.path.join(PLOT_DIR, f'nxm_pca_components_zip{det_tmpl}.png')
    fig_pca.savefig(pca_path, dpi=150)
    plt.close(fig_pca)
    print(f"PCA components plot saved: {pca_path}")

    # Variance explained plot
    fig_var, ax_var = plt.subplots(figsize=(6, 4))
    ax_var.plot(range(N_COMPONENTS), var * 100, 'o-', color='steelblue')
    ax_var.set_xlabel('Component index')
    ax_var.set_ylabel('Explained variance (%)')
    ax_var.set_title(f'Zip{det_tmpl} NxM — Explained variance')
    ax_var.set_yscale('log')
    ax_var.grid(alpha=0.4)
    fig_var.tight_layout()
    var_path = os.path.join(PLOT_DIR, f'nxm_variance_zip{det_tmpl}.png')
    fig_var.savefig(var_path, dpi=120)
    plt.close(fig_var)

    # Write NxM templates — full 32768-sample histograms (no padding needed)
    ...
    print(f"NxM templates ({N_COMPONENTS} components × {len(chan_names)} channels) "
          f"written to ROOT.")
