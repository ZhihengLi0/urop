...
- `..._cumulative_energy_PBS1_15events.png` / `..._cumulative_energy_allchan_ev30646.png`
  — **功率的累积积分曲线(eV)**,直接回答"积出来多大":曲线拉平的高度就是吸收能量,
  绿虚线 = 官方窗末的值,紫点线 = 16 ms 处的值。叠加图里另加了**绿色等面积横线**
  (窗内平均功率,其矩形面积 = 能量,PBS1 ev30646 为 32 fW × 1.5 ms = 300 eV)。

**累积曲线揭示的新信息(重要)**:积分在触发后 1–2 ms 内就爬到平台,之后**不是保持水平,
而是上下漂 ±20%**;逐事件差异极大(ev 210571 从 259 eV 一路掉到 34 eV,ev 190030
270→146 eV)。全 11 通道同一事件的累积曲线**漂移形状完全同相**(5 ms 处同时下凹、
7.5–12 ms 处同时上凸,PDS2 幅度最大 80→570 eV)⇒ 这是**共模基线摆动**,不是各通道
独立噪声。两点结论:
1. 这就是 §6f "暴力放宽窗口不收敛"的直接图像证据,进一步支持"中窗 + 解析尾巴外推";
2. 因为是共模的,**共模扣除(用全通道平均漂移做基线校正)有可能救回长窗积分**,
   是一个值得试的新方向。
...
`scripts/plot_fitted_current_power.py` → 同一个 results 目录,文件名带 `fitted_`。
