...
**中文**：大家好，我叫李志恒，来自明尼苏达大学。今天讲 Z7 的声子收集效率。问题是：一个 10.37 keV 的事件发生在晶体里，有多少能量被 TES 吸收。我们得到的结果是 30.5%，用的是 10 个通道；另外两个排除了，一个数据缺失，一个噪声太大。我先用一个脉冲说明能量是怎么算的，再把同样的方法用到整个探测器。
