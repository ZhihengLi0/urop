#!/usr/bin/env python3
"""Event-organized fit examples: 15 events x all 12 channels in one grid.

Each row is ONE event, each column one phonon channel; every panel shows the
100 kHz LP measured trace (blue) and that channel's free-pretrigger 2-exp fit
(red) with its NRMSE. Events are the first 15 selected events found in the
raw cache (storage order, NOT quality-selected), so the grid shows honestly
how the same physical event looks and fits across all channels.

Overwrites results/plots/fit_examples/zip{N}_fit_examples.png (this format
supersedes the old per-channel first-3 layout).

Usage:
    python3 plot_event_fit_grid.py --det 7 [--n-events 15]
"""
...
from lp_fit_align import (add_pipeline_note, plot_path,
                          ...
                          SAMPLERATE, X_FULL, normalize_trace, two_exp_free_pt)
...
# ── collect the first N events (storage order) with per-channel trace + fit ──
events = []           # list of (event_number, series, {chan: (y_norm, fp)})
for series in series_list:
    ...
    # index of each event within each channel's trace list
    ...
    for evn in payload.get("selected_event_numbers", []):
        ...
        for c in ALL_CHANS:
            ...
            if i is None:
                continue
            y_norm = normalize_trace(payload["raw_traces"][c][i])
            if y_norm is None:
                continue
            fp = (fits.get(c) or [None] * (i + 1))[i] \
                if i < len(fits.get(c) or []) else None
            per_chan[c] = (y_norm, fp)
        if per_chan:
...
# ── grid figure: rows = events, columns = channels ────────────────────────────
...
fig.suptitle(f"Zip{det} — first {len(events)} selected events x all channels: "
             "LP trace (blue) vs 2-exp fit (red)", fontsize=13)
for row, (evn, series, per_chan) in enumerate(events):
    for col, c in enumerate(ALL_CHANS):
        ...
        if c not in per_chan:
            ...
            continue
        y_norm, fp = per_chan[c]
        ax.plot(t_ms, y_norm[lo:hi], lw=0.4, color="steelblue")
        if fp is not None:
...
fig.tight_layout()
add_pipeline_note(fig, "event-organized fit examples: one ROW per event (first "
                  f"{len(events)} selected events in storage order, NOT quality-"
                  "selected), one COLUMN per channel; blue = measured LP trace, "
                  "red = that channel's free-pretrigger 2-exp fit; top-right of "
                  "each panel = NRMSE ('!ok' = failed fit_ok physicality check)")
