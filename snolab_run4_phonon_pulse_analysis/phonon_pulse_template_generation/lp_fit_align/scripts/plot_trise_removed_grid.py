...
fig, axes = plt.subplots(nrows, ncols, figsize=(2.6 * ncols, 1.9 * nrows),
                         squeeze=False)
fig.suptitle(f"Zip{det} — events that PASS NRMSE <= {CUT} but are REMOVED by "
             f"the tau_rise <= {TRISE*1e3:.2f} ms cut, first {len(events)} "
             "(median t_rise > cut): raw (gray) vs LP (blue) vs 2-exp fit (red)",
             fontsize=13)
...
fig.tight_layout()
add_pipeline_note(fig, f"Events that PASS the NRMSE cut (median NRMSE <= {CUT}) but are "
                  f"REMOVED by the tau_rise <= {TRISE*1e3:.2f} ms ceiling (median t_rise > "
                  f"{TRISE*1e3:.2f} ms), first {len(events)} in storage order; these slipped "
                  "past 0.4 yet carry no clean fast pulse - a slow 2-exp hugs a slow baseline "
                  "drift; one ROW per event, one COLUMN per channel, gray = raw, blue = 100kHz "
                  "LP, red = 2-exp fit")
out = args.out or plot_path("shadow_events", f"zip{det}_shadow_events.png")
